...
sub new {
    ...
    my $config;
    if ( $_[0] && ref $_[0] && ref $_[0] eq 'Kubernetes::Configuration' ) {
        $config = $_[0];
    } else {
        $config = Kubernetes::Configuration->new(@_);
    }
...
sub deserialize
{
    ...
    if (not defined $data) {
        ...
        my $_instance = use_module("Kubernetes::Object::$class")->new;
        if (ref $data eq "HASH") {
